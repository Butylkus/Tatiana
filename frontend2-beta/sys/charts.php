...
if (!preg_match('/^[1-9]$|^[1,2][0-7]$/', $_GET['pin']) or !is_numeric($_GET['pin'])){
    ...
    $pin=$_GET['pin'];
    $data = mysql_query("SELECT name FROM pins WHERE pin='$pin';");
    while ($row = mysql_fetch_assoc($data)) {
        $pinname=$row["name"]; //pin[№]=="имя"...
    }
}
...
//Собираем конечный код осей
    ...
    $returner = str_replace("%DHTNAME%",$pinname,$returner);
...
///////////////////////////////////////////
if ($_GET['type']=="temp" and $_GET['period']=="month"){
    $querypin=$_GET['pin'];
    $data = mysql_query("SELECT temperature, timestamp FROM dht_data WHERE pin=$querypin ORDER BY id DESC LIMIT 1441;");    

//лепим массив данных. всего 1440 значений (0-48)
    $i=1440;
    while ($row = mysql_fetch_assoc($data)) {
        $temp[$i]=400-$row["temperature"]*10;//координаты по У, температура. Диапазон от 0 до 40 градусов
        $time[$i]=$row["timestamp"];//координаты по Х, время. координата вычисляется дальше
...
// переворачиваем массив, чтобы график шёл слева направо
    ...
    for ($i=0;$i<=1440;$i+=2){
        $x = round(20+.666*$i); //вычисляем иксовую координату времени
        $polypoints = $polypoints."$x,$temp[$i] "; //%POLYPOINTS% - Собираем готовые координаты в строку 
    }

    $currdotval = (400-$temp[1440])/10; //%CURRENTDOTVALUE%
    $rotatedotx = $x+4; //%ROTATEDOTX%
    $rotatedoty = $temp[1440]-14; //%ROTATEDOTY%

//Собираем конечный код осей
    for ($i=0;$i<=1440;$i=$i+48){
        $day=round(38+.666*$i); //вычисляем иксовую координату подписи даты
        $dayaxis = $dayaxis     .'<text class="xtime" x="'.$day.'" y="399" fill="#aaa" transform="rotate(-90 '.$day.',399)">'.date('d.m',$time[$i])."</text>\n"; //%DAYAXIS%
    }
       
    $curdateval = date("H:i:s, d.m.Y",$time[1440]); //%CURRENTDOTDATATIME%
    $template = file_get_contents('../template/charts/tempmonth.txt');
    $returner = str_replace("%CURRENTDOTX%",$x,$template);
    $returner = str_replace("%CURRENTDOTY%",$temp[1440],$returner);
    $returner = str_replace("%CURRENTDOTVALUE%",$currdotval,$returner);
    $returner = str_replace("%DHTNAME%",$pinname,$returner);
    $returner = str_replace("%CURRENTDOTDATATIME%",$curdateval,$returner);
    $returner = str_replace("%ROTATEDOTX%",$rotatedotx,$returner);
...
/////////////МЕСЯЧНАЯ ВЛАЖНОСТЬ////////////
///////////////////////////////////////////
if ($_GET['type']=="hmdt" and $_GET['period']=="month"){
    $querypin=$_GET['pin'];
    $data = mysql_query("SELECT humidity, timestamp FROM dht_data WHERE pin=$querypin ORDER BY id DESC LIMIT 1441;");    
...
// переворачиваем массив, чтобы график шёл слева направо
    ...
    $currdotval = (400-$hmdt[1440])/4; //%CURRENTDOTVALUE%
    $rotatedotx = $x+4; //%ROTATEDOTX%
    $rotatedoty = $hmdt[1440]-14; //%ROTATEDOTY%
...
//Собираем конечный код осей
    ...
    $curdateval = date("H:i:s, d.m.Y",$time[1440]); //%CURRENTDOTDATATIME%
    $template = file_get_contents('../template/charts/hmdtmonth.txt');
    $returner = str_replace("%CURRENTDOTX%",$x,$template);
    $returner = str_replace("%CURRENTDOTY%",$hmdt[1440],$returner);
    $returner = str_replace("%CURRENTDOTVALUE%",$currdotval,$returner);
    $returner = str_replace("%DHTNAME%",$pinname,$returner);
    $returner = str_replace("%CURRENTDOTDATATIME%",$curdateval,$returner);
    $returner = str_replace("%ROTATEDOTX%",$rotatedotx,$returner);
